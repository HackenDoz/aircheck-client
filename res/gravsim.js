...
Sim.update = function() {
		...
		for (var i = 0; i < Sim.blobs.length; ++i) {
			...
			if (xd * xd + yd * yd < blob.size * blob.size) {
				...
				for(var j = 0; j < toSplit; ++j) {
					...
					Sim.addBlob(blob.x + xx * (blob.size + sizeToExplode / toSplit + dist), blob.y + yy * (blob.size + sizeToExplode / toSplit + dist), sizeToExplode / toSplit, massToExplode / toSplit, xx * 1000, yy * 1000);
				}
				break;
			}
		}
	...
	if (hold) {
		Sim.addBlob((mousex - transX) / scale, (mousey - transY) / scale, 10, 100, 0, 0);
	}
	if (drag != null) {
